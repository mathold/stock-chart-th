...
SIG_BUY = "#FF2A2A"             # แดง   = สัญญาณซื้อ
SIG_SELL = "#FFE800"            # เหลือง = สัญญาณขาย
SIG_HOLD = "#2A44E0"            # น้ำเงิน = ถือหุ้น
SIG_CASH = "#00D5F5"            # ฟ้า    = ถือเงินสด

# ── Pattern 49 ── นับแท่งเทียบกับ 4 แท่งก่อนหน้า ครบ 9 แล้วเริ่มใหม่
P49_LOOKBACK, P49_TARGET = 4, 9
P49_UP_COLOR = "#FF4FD8"        # ชมพู  = นับฝั่งขึ้น (อยู่เหนือแท่ง)
P49_DOWN_COLOR = "#00E36A"      # เขียว = นับฝั่งลง  (อยู่ใต้แท่ง)
...
MCD_PROFIT = "#E00000"          # แดง   = ต้นทุนต่ำกว่าราคา (กำไร / รายใหญ่)
MCD_FLOAT = "#FFE800"           # เหลือง = ต้นทุนใกล้ราคา  (ลอย / รายย่อย)
MCD_TRAP = "#00D000"            # เขียว = ต้นทุนสูงกว่าราคา (ติดดอย)
MCD_MA_PROFIT = "#B060FF"       # ม่วง  ตามแท่งแดง
...
def mcd(df: pd.DataFrame) -> pd.DataFrame:
    ...
    จากนั้นแบ่งตามราคาปัจจุบัน (MCD_BAND = ครึ่งความกว้างของช่วง "ลอย")
      ต่ำกว่าราคามาก = กำไร   (แดง)
      ใกล้ราคา       = ลอย    (เหลือง)
      สูงกว่าราคามาก = ติดดอย (เขียว)
